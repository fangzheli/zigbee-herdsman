// biome-ignore-all lint/suspicious/noExplicitAny: Tests build dynamic Vitest mocks around the adapter driver.
...
import type {AdapterOptions, NetworkOptions, SerialPortOptions, StartResult} from "../../../../src/adapter/tstype";
...
describe("BLZ Adapter", () => {
    ...
    beforeEach(async () => {
        ...
        driverMock.formNetworkWithParameters.mockImplementation((extendedPanId: bigint, panId: number, channel: number) =>
    ...
    describe("Startup and initialization", () => {
        ...
        it("should reject adapter waiters with the startup failure when startup fails", async () => {
            ...
            await expect(adapter.start()).rejects.toThrow("startup failed");
            const observed = await Promise.race([waiterResult, Promise.resolve("pending")]);
        ...
        it("should cancel the startup settle delay when stopping", async () => {
            ...
            await adapter.stop();
            await vi.advanceTimersByTimeAsync(0);
            const observed = await Promise.race([startResult, Promise.resolve("pending")]);
        ...
        it("should coalesce concurrent adapter starts and cancel them together", async () => {
            ...
            expect(observed).toEqual(["rejected:Adapter stopped", "rejected:Adapter stopped"]);
            expect(driverMock.stop).toHaveBeenCalled();
        });
        ...
        it("should reject queued adapter jobs when stopping", async () => {
            ...
            const firstSend = adapter.sendZclFrameToEndpoint("0x0102030405060708", 0x1234, 1, zclFrame, 1000, true, true);
            const secondSend = adapter.sendZclFrameToEndpoint("0x0102030405060709", 0x1235, 1, zclFrame, 1000, true, true);
            ...
            await adapter.stop();
            const observed = await Promise.race([secondResult, new Promise((resolve) => setImmediate(() => resolve("pending")))]);
        ...
        it("should keep rejecting new adapter requests after a failed stop", async () => {
            ...
            const observed = await Promise.race([result, new Promise((resolve) => setImmediate(() => resolve("pending")))]);
    ...
    describe("Network operations", () => {
        ...
        it("should cancel coordinator permit join when stopping", async () => {
            ...
            await adapter.stop();
            await vi.advanceTimersByTimeAsync(0);
            const observed = await Promise.race([permitJoinResult, Promise.resolve("pending")]);
        ...
        it("should send ZCL frame to all through the broadcast driver API", async () => {
            ...
            const send = adapter.sendZclFrameToAll(3, zclFrame, 1, ZSpec.BroadcastAddress.DEFAULT);
        ...
        it("should cancel group send settle delay when stopping", async () => {
            ...
            await adapter.stop();
            await vi.advanceTimersByTimeAsync(0);
            const observed = await Promise.race([sendResult, Promise.resolve("pending")]);
            await vi.advanceTimersByTimeAsync(200);
            await send.catch(() => {});
        ...
        it("should register active group lower sends as cancellable adapter operations", async () => {
            ...
            const runningOperations = (adapter as unknown as {runningOperations: {count: () => number}}).runningOperations;
        ...
        it("should not retry endpoint ZCL response waits after stop clears waiters", async () => {
            ...
            const send = adapter.sendZclFrameToEndpoint("0x0102030405060708", 0x1234, 1, zclFrame, 1000, false, false);
            const sendResult = send.then(
                () => "resolved",
                (error: Error) => `rejected:${error.message}`,
            );
        ...
        it("should cancel endpoint ZCL response waiters when the driver endpoint send throws", async () => {
            ...
            await expect(adapter.sendZclFrameToEndpoint("0x0102030405060708", 0x1234, 1, zclFrame, 1000, false, true)).rejects.toThrow(
                "driver endpoint send failed",
            );

            expect((adapter as unknown as {zclResponseWaiters: {count: () => number}}).zclResponseWaiters.count()).toBe(0);
        });
        ...
        it("should register active endpoint lower sends as cancellable adapter operations", async () => {
            ...
            const send = adapter.sendZclFrameToEndpoint("0x0102030405060708", 0x1234, 1, zclFrame, 1000, true, true);
            const sendResult = send.then(
                () => "resolved",
                (error: Error) => `rejected:${error.message}`,
            );
        ...
        it("should log endpoint ZCL retry state without stale data-request attempts", async () => {
            ...
            try {
                ...
                await adapter.sendZclFrameToEndpoint("0x0102030405060708", 0x1234, 1, zclFrame, 1000, true, true);

                const endpointLog = debug.mock.calls
                    .map(([message]) => (typeof message === "function" ? message() : message))
                    .find((message) => String(message).startsWith("sendZclFrameToEndpointInternal"));
        ...
        it("should not stringify endpoint ZCL send details unless debug logging evaluates the message", async () => {
            ...
            try {
                ...
                await adapter.sendZclFrameToEndpoint(ieeeAddr, 0x1234, 1, zclFrame, 1000, true, true);
        ...
        it("should not stringify endpoint ZCL timeout details unless debug logging evaluates the message", async () => {
            ...
            try {
                ...
                const send = adapter.sendZclFrameToEndpoint(ieeeAddr, 0x1234, 1, zclFrame, 1000, false, true);
                const sendResult = send.then(
                    () => "resolved",
                    (error: Error) => `rejected:${error.message}`,
                );
                ...
                await expect(sendResult).resolves.not.toBe("rejected:eager endpoint ZCL timeout detail string");
                expect(debug).toHaveBeenCalledWith(expect.any(Function), expect.any(String));
                void send.catch(() => {});
            } finally {
                debug.mockRestore();
        ...
        it("should use source endpoint and profile ID for group ZCL sends", async () => {
            ...
            expect(driverMock.sendZclMulticast).toHaveBeenCalledWith(0x1234, Zcl.Clusters.genOnOff.ID, 0x0105, 5, zclFrame.toBuffer());
        });
        ...
        it("should hand parsed channel changes to the driver API", async () => {
            ...
            const payload = Zdo.Buffalo.buildRequest(true, Zdo.ClusterId.NWK_UPDATE_REQUEST, [15], 0xfe, undefined, 1, undefined);

            const change = adapter.sendZdo(ZSpec.BLANK_EUI64, ZSpec.BroadcastAddress.SLEEPY, Zdo.ClusterId.NWK_UPDATE_REQUEST, payload, true);
        ...
        it("should not copy the raw NWK update payload just for logging", async () => {
            ...
            ).handleNwkUpdateRequest(ZSpec.BroadcastAddress.SLEEPY, Zdo.ClusterId.NWK_UPDATE_REQUEST, payload, true);
        ...
        it("should serialize concurrent channel changes through the adapter queue", async () => {
            ...
            const firstPayload = Zdo.Buffalo.buildRequest(true, Zdo.ClusterId.NWK_UPDATE_REQUEST, [15], 0xfe, undefined, 1, undefined);
            const secondPayload = Zdo.Buffalo.buildRequest(true, Zdo.ClusterId.NWK_UPDATE_REQUEST, [20], 0xfe, undefined, 2, undefined);
        ...
        it("should route leave ZDO requests through the driver API", async () => {
            ...
            driverMock.sendZdo.mockImplementation(async (ieee: string, nwk: number) => {
                driverMock.on.mock.calls.find((call) => call[0] === "deviceLeft")?.[1](nwk, {toString: () => ieee.replace(/^0x/, "")});
            });

            await adapter.sendZdo("0x0102030405060708", 0x1234, Zdo.ClusterId.LEAVE_REQUEST, Buffer.from([0, 1, 2, 3]), true);
    ...
    describe("Message handling", () => {
        it("should handle device join", () => {
            ...
            driverMock.on.mock.calls.find((call) => call[0] === "deviceJoined")?.[1](0x1234, {toString: () => "0x0102030405060708"});
        ...
        it("should not resolve ZCL waiters from frames with reserved frame types", async () => {
            ...
            driverMock.on.mock.calls.find((call) => call[0] === "incomingMessage")?.[1]({
                apsFrame,
                message: Buffer.from([0x02, 7, Zcl.Clusters.genOnOff.commands.toggle.ID]),
                sender: 0x1234,
                lqi: 255,
            });
        ...
        it("should mark broadcast ZCL messages", () => {
            ...
            expect(callback).toHaveBeenCalledWith(expect.objectContaining({wasBroadcast: true}));
        });
        ...
        it("should resolve ZCL waiters with default responses like the shared adapter matcher", async () => {
            ...
            await driverMock.on.mock.calls.find((call) => call[0] === "incomingMessage")?.[1]({
                apsFrame,
    ...
    describe("Backup operations", () => {
        ...
        it("should cancel backup when stopping", async () => {
            ...
            await adapter.stop();
            await vi.advanceTimersByTimeAsync(0);
            const observed = await Promise.race([backupResult, Promise.resolve("pending")]);
        ...
        it("should pass a stop guard that prevents backup continuation after stop", async () => {
            ...
            driverMock.createBackup.mockImplementation(async (assertActive?: () => void) => {
    ...
    describe("Error handling", () => {
        it("should propagate driver-owned ZDO send failures", async () => {
            ...
            await expect(
                adapter.sendZdo("0x0102030405060708", 0x1234, Zdo.ClusterId.NODE_DESCRIPTOR_REQUEST, Buffer.from([0x00, 0x34, 0x12]), false),
        ...
        it("should pass caller-owned ZDO payload buffers to the driver without cloning", async () => {
            ...
            try {
                await adapter.sendZdo("0x0102030405060708", 0x1234, Zdo.ClusterId.NODE_DESCRIPTOR_REQUEST, payload, true);

                expect(payload).toEqual(Buffer.of(0xaa, 0xbb, 0xcc));
                expect(driverMock.sendZdo).toHaveBeenCalledWith("0x0102030405060708", 0x1234, Zdo.ClusterId.NODE_DESCRIPTOR_REQUEST, payload, true);
        ...
        it("should not finish active ZDO sends after stop interrupts the driver request", async () => {
            ...
            const send = adapter.sendZdo("0x0102030405060708", 0x1234, Zdo.ClusterId.LEAVE_REQUEST, Buffer.from([0x00]), true);
            const sendResult = send.then(
                () => "resolved",
                (error: Error) => `rejected:${error.message}`,
            );
