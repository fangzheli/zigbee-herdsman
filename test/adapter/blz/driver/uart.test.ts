// biome-ignore-all lint/suspicious/noExplicitAny: Tests build dynamic Vitest mocks around serial/socket ports.
...
describe("BLZ Serial Driver", () => {
    ...
    describe("Connection", () => {
        it("should keep low-level UART waiters behind driver internals", () => {
            ...
            expect(source).toContain("private handleError(_frame: Frame): void");
        ...
        it("should not stringify serial open options unless debug logging evaluates the message", async () => {
            ...
            const stringify = vi.spyOn(JSON, "stringify").mockImplementation((value: unknown) => {
                if (typeof value === "object" && value !== null && (value as {path?: string}).path === serialPortOptions.path) {
        ...
        it("should reject pending serial connect when closing before open finishes", async () => {
            ...
            const observed = await Promise.race([connectResult, new Promise((resolve) => setImmediate(() => resolve("pending")))]);
        ...
        it("should close serial port when connect operation cleanup fails during close", async () => {
            ...
            await driver.connect(serialPortOptions);
            const connectOperations = (driver as unknown as {connectOperations: {cancel: (error: Error) => void}}).connectOperations;
        ...
        it("should release the serial port reference when flush close fails", async () => {
            serialPortMock.asyncOpen.mockResolvedValue(undefined);
            serialPortMock.asyncFlushAndClose.mockRejectedValue(new Error("Flush close failed"));
        ...
        it("should clean listeners and pipes when TCP socket open fails", async () => {
            ...
            socketPortMock.once.mock.calls.find((call) => call[0] === "error")?.[1](new Error("Socket failed"));
        ...
        it("should reject and clean listeners when TCP socket closes before ready", async () => {
            const connect = driver.connect(tcpPortOptions);
            const closeBeforeReady = socketPortMock.once.mock.calls.find((call) => call[0] === "close")?.[1];
        ...
        it("should stay closed when TCP connect is closed while ready reset is pending", async () => {
            ...
            const ready = socketPortMock.on.mock.calls.find((call) => call[0] === "ready")?.[1];
    ...
    describe("Frame handling", () => {
        ...
        it("should not stringify parsed ACK frames unless debug logging evaluates the message", async () => {
            ...
            const waiter = (
                driver as unknown as {
                    frameWaiters: {
                        waitFor: (frameId: number, timeout?: number) => {start: () => {promise: Promise<unknown>; ID: number}; ID: number};
            ...
            try {
                ...
                (driver as unknown as {frameWaiters: {cancel: (waiter: {ID: number}) => void}}).frameWaiters.cancel(waiter);
                await waiterResult;
                toStringSpy.mockRestore();
                debug.mockRestore();
                error.mockRestore();
        ...
        it("should clear pending waiters and emit reset when a RESET frame is received", async () => {
            ...
            parserMock.on.mock.calls.find((call) => call[0] === "parsed")?.[1](createFrame(0x0003, 0x01, 0x00));
            const observed = await Promise.race([waiterResult, new Promise((resolve) => setImmediate(() => resolve("pending")))]);
        ...
        it("should handle ERROR frames", async () => {
            ...
            // ERROR frames are handled by the reset lifecycle path.
            expect(writerMock.sendACK).not.toHaveBeenCalled();
        });

        it("should clear pending waiters and emit reset when an ERROR frame is received", async () => {
            const callback = vi.fn();
            driver.on("reset", callback);
            const waiter = (
                driver as unknown as {
            ...
            parserMock.on.mock.calls.find((call) => call[0] === "parsed")?.[1](createFrame(0x0002, 0x01, 0x00));
            const observed = await Promise.race([waiterResult, new Promise((resolve) => setImmediate(() => resolve("pending")))]);
        ...
        it("should not retain unused parser reject state after malformed frames", () => {
            ...
            expect("rejectCondition" in (driver as unknown as Record<string, unknown>)).toBe(false);
        });
    ...
    describe("Data sending", () => {
        ...
        it("should send data successfully", async () => {
            ...
            expect(writerMock.sendData).toHaveBeenCalledWith(data, expect.any(Number), expect.any(Number), frameId, false, false);
        });
        ...
        it("should serialize concurrent DATA sends before ACK", async () => {
            ...
            parserMock.on.mock.calls.find((call) => call[0] === "parsed")?.[1](createFrame(frameId, 0x01, 0x00));
            await firstSend;
            await Promise.resolve();
            ...
            parserMock.on.mock.calls.find((call) => call[0] === "parsed")?.[1](createFrame(frameId, 0x02, 0x00));
            await secondSend;
        });
        ...
        it("should clear UART waiters when queue cleanup fails during close", async () => {
            ...
            expect(error).toBe(cleanupError);
            expect(observed).toBe("rejected:Connection closed");
            expect((driver as unknown as {frameWaiters: {count: () => number}}).frameWaiters.count()).toBe(0);
            expect(serialPortMock.asyncFlushAndClose).toHaveBeenCalled();
        });
        ...
        it("should handle send failure with retries", async () => {
            ...
            try {
                ...
                const rejection = expect(send).rejects.toThrow("Failed to send data after 1 retries");

                await vi.advanceTimersByTimeAsync(3000);
                await rejection;
                expect(writerMock.sendData).toHaveBeenCalledTimes(2); // Initial + 1 retry
                expect(writerMock.sendData).toHaveBeenNthCalledWith(1, data, expect.any(Number), expect.any(Number), frameId, false, false);
                expect(writerMock.sendData).toHaveBeenNthCalledWith(2, data, expect.any(Number), expect.any(Number), frameId, true, false);
            } finally {
                vi.useRealTimers();
            }
        });
        ...
        it("should reject pending send waiters when resetting", async () => {
            ...
            await driver.reset();
            const observed = await Promise.race([sendResult, new Promise((resolve) => setImmediate(() => resolve("pending")))]);
        ...
        it("should handle synchronous writer failures before waiters start", async () => {
            ...
            await expect(driver.sendDATA(Buffer.from([1, 2, 3]), 0x0000, 0)).rejects.toThrow("Failed to send data after 0 retries");
        });
