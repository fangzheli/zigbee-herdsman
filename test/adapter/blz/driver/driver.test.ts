...
describe("BLZ high-level driver lifecycle", () => {
    ...
    it("keeps the lower BLZ transport behind driver APIs", () => {
        ...
        expect(source).toContain("private cancelResetRecovery(error: Error): void");
        expect(source).toContain("this.cancelResetRecovery(error);");
        ...
        expect(source.match(/this\.startupDelay\.cancel\(\);/g)).toHaveLength(1);
        expect(source).toContain("private cancelStartupRunningOperations(error: Error): void");
        expect(source).toContain("this.cancelStartupRunningOperations(error);");
        expect(source.match(/this\.startupOperations\.cancel\(error\);/g)).toHaveLength(1);
        ...
        expect(source).toContain("private detachBlzRuntimeListeners(blz: Blz): void");
        expect(source).toContain("this.attachBlzCloseListener(blz);");
        expect(source).toContain("this.attachBlzRuntimeListeners(blz);");
        expect(source).toContain("this.detachBlzCloseListener(blz);");
        expect(source).toContain("this.detachBlzRuntimeListeners(blz);");
    ...
    it.each([
        ...
        {
            name: "addEndpoint",
            invoke: (driver: Driver) => driverAddEndpoint(driver, {endpoint: 1}),
    ...
    it("cancels startup delay when stop interrupts startup", async () => {
        ...
        blzConstructorMock.mockImplementation(() => blzMock);
        const driver = new Driver(serialPortOptions, networkOptions, "/tmp/backup.json");
        const addEndpoint = spyOnDriverAddEndpoint(driver).mockResolvedValue(undefined);
    ...
    it("does not stringify startup coordinator IEEE unless debug logging evaluates the message", async () => {
        ...
        spyOnDriverAddEndpoint(driver).mockResolvedValue(undefined);
        const toStringSpy = vi.spyOn(BlzEUI64.prototype, "toString").mockImplementation(() => {
            throw new Error("eager startup coordinator IEEE string");
        });
    ...
    it("cancels startup while addEndpoint is pending", async () => {
        ...
        blzConstructorMock.mockImplementation(() => blzMock);
        const driver = new Driver(serialPortOptions, networkOptions, "/tmp/backup.json");
        const addEndpoint = spyOnDriverAddEndpoint(driver).mockReturnValue(new Promise<void>(() => {}));
