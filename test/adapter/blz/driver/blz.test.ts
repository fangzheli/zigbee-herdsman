// biome-ignore-all lint/suspicious/noExplicitAny: Tests build dynamic Vitest mocks around BLZ frame parsing.
// biome-ignore-all lint/suspicious/useAwait: Some Vitest callbacks intentionally keep async promise shape.

import * as fs from "node:fs";

import {afterEach, beforeEach, describe, expect, it, vi} from "vitest";
import {type BLZFrameData, Blz, MAX_SERIAL_CONNECT_ATTEMPTS, NS, SERIAL_CONNECT_NEW_ATTEMPT_MIN_DELAY} from "../../../../src/adapter/blz/driver/blz";
...
import type {SerialPortOptions} from "../../../../src/adapter/tstype";
...
describe("BLZ Driver", () => {
    ...
    describe("Connection", () => {
        ...
        it("should close the existing serial driver before reconnecting", async () => {
            serialDriverMock.connect.mockResolvedValue(undefined);
            serialDriverMock.isInitialized.mockReturnValueOnce(false).mockReturnValueOnce(true).mockReturnValueOnce(true).mockReturnValueOnce(true);
            serialDriverMock.close.mockResolvedValue(undefined);
        ...
        it("should close the existing serial driver when reconnect runtime cleanup fails", async () => {
            ...
            serialDriverMock.connect.mockResolvedValue(undefined);
            serialDriverMock.isInitialized.mockReturnValueOnce(false).mockReturnValueOnce(true).mockReturnValueOnce(true);
            serialDriverMock.close.mockResolvedValue(undefined);
        ...
        it("should clear the previous watchdog timer when reconnect attempts fail", async () => {
            ...
            await rejection;
            expect(clearIntervalSpy).toHaveBeenCalledTimes(1);
            expect(serialDriverMock.connect).toHaveBeenCalledTimes(MAX_SERIAL_CONNECT_ATTEMPTS + 1);
        });
        ...
        it("should report both connect and connect-reset listener cleanup failures", async () => {
            ...
            expect((aggregateError.errors[0] as Error).message).toBe(`Failed to connect after ${MAX_SERIAL_CONNECT_ATTEMPTS} attempts`);
        ...
        it("should detach the existing serial reset listener when reconnect attempts fail", async () => {
            ...
            await blz.connect(serialPortOptions);
            const runtimeResetHandler = serialDriverMock.on.mock.calls.filter((call) => call[0] === "reset").at(-1)?.[1];
            serialDriverMock.connect.mockRejectedValue(new Error("Connection failed"));
            serialDriverMock.off.mockClear();
        ...
        it("should preserve failed-attempt cleanup when connect errors cannot expose a message", async () => {
            ...
            const connect = blz.connect(serialPortOptions).catch((caught: Error) => caught);
            ...
            expect(error.message).toBe(`Failed to connect after ${MAX_SERIAL_CONNECT_ATTEMPTS} attempts`);
            expect(error.cause).toBe(connectError);
            expect(serialDriverMock.connect).toHaveBeenCalledTimes(MAX_SERIAL_CONNECT_ATTEMPTS);
            expect(serialDriverMock.close).toHaveBeenCalledTimes(MAX_SERIAL_CONNECT_ATTEMPTS);
            expect(errorLog).toHaveBeenCalledWith("Log message formatting failed: Error: connect message stringification failed", NS);
        });
        ...
        it("should cancel connection retry waits when closing", async () => {
            ...
            await blz.close(false);
            await vi.advanceTimersByTimeAsync(0);
            const observed = await Promise.race([connectResult, Promise.resolve("pending")]);
        ...
        it("should cancel reconnect while closing the existing serial driver", async () => {
            ...
            const close = blz.close(false);
            await vi.advanceTimersByTimeAsync(0);
            const observed = await Promise.race([reconnectResult, Promise.resolve("pending")]);
        ...
        it("should convert reset events during connect into a failed attempt", async () => {
            ...
            const resetForReconnect = serialDriverMock.on.mock.calls.filter((call) => call[0] === "reset").at(-1)?.[1];
            ...
            await expect(connectResult).resolves.toBe("rejected:Connection cancelled by close");
        });

        it("should close the serial driver before retrying a resolved but uninitialized connection", async () => {
            serialDriverMock.connect.mockResolvedValue(undefined);
            serialDriverMock.isInitialized.mockReturnValueOnce(false).mockReturnValueOnce(false).mockReturnValueOnce(true);
            serialDriverMock.close.mockResolvedValue(undefined);
    ...
    describe("Value operations", () => {
        ...
        it("should reject getValue when the command status is not success", async () => {
            ...
            await expect(blz.getValue(BlzValueId.BLZ_VALUE_ID_STACK_VERSION)).rejects.toThrow(
                "Failed to get value BLZ_VALUE_ID_STACK_VERSION: status 1",
            );
        });
        ...
        it("should reject setValue when the command status is not success", async () => {
            ...
            await expect(blz.setValue(BlzValueId.BLZ_VALUE_ID_STACK_VERSION, 0x12345678)).rejects.toThrow(
                "Failed to set value BLZ_VALUE_ID_STACK_VERSION: status 1",
            );
        });
        ...
        it("should not retain waiters for reset commands that do not wait for response", async () => {
            ...
            expect((blz as unknown as {commandWaiters: {count: () => number}}).commandWaiters.count()).toBe(0);
        });
        ...
        it("should not throw when matching an unknown numeric frame waiter", () => {
            const result = (
                blz as unknown as {
                    commandWaiters: {
                        matches: (payload: {frameName: string}, matcher: {frameId: number}) => boolean;
                    };
                }
            ).commandWaiters.matches({frameName: "getValue"}, {frameId: 0xffff});
        ...
        it("should match string frame waiters without array matching churn", () => {
            ...
            try {
                result = (
                    blz as unknown as {
                        commandWaiters: {
                            matches: (payload: {frameName: string}, matcher: {frameId: string}) => boolean;
                        };
                    }
                ).commandWaiters.matches({frameName: "getValue"}, {frameId: "getValue"});
            } finally {
                includesSpy.mockRestore();
            }
        ...
        it("should cancel active commands before forcing a UART reset", async () => {
            ...
            const observed = await Promise.race([commandResult, Promise.resolve("pending")]);
        ...
        it("should restore reset state when force reset cleanup fails before UART reset", async () => {
            ...
            serialDriverMock.isInitialized.mockReturnValue(true);
            vi.spyOn(blz as unknown as {clearPendingCommands: (error: Error) => void}, "clearPendingCommands").mockImplementation(() => {
                throw cleanupError;
            });
    ...
    describe("Event handling", () => {
        ...
        const waitForCommandResponse = (): {
            ...
            (
                blz as unknown as {
                    commandWaiters: {
                        waitFor: (frameId: string, timeout?: number) => {start: () => {promise: Promise<unknown>; ID: number}; ID: number};
                    };
                }
        ...
        it("should clear BLZ waiters with the reset reason on serial driver reset", async () => {
            ...
            const resetHandler = serialDriverMock.on.mock.calls.filter((call) => call[0] === "reset").at(-1)?.[1];
        ...
        it("should ignore malformed received buffers that are too short for a BLZ frame", () => {
            ...
            try {
                ...
                expect(error).toHaveBeenCalledWith("Received malformed BLZ frame: expected at least 6 bytes, received 3", NS);
            } finally {
                error.mockRestore();
            }
        });
        ...
        it("should ignore received frames that cannot be decoded", () => {
            ...
            try {
                expect(() => receivedHandler(data)).not.toThrow();
                expect(frame).not.toHaveBeenCalled();
                expect(error).toHaveBeenCalledWith("Failed to parse BLZ frame 0xffff: Error: Unrecognized frame FrameID 65535", NS);
        ...
        it("should ignore received frames whose decode errors cannot be stringified", () => {
            ...
            try {
                expect(() => receivedHandler(data)).not.toThrow();
                expect(frame).not.toHaveBeenCalled();
                expect(error).toHaveBeenCalledWith("Log message formatting failed: Error: decode stringification failed", NS);
            } finally {
