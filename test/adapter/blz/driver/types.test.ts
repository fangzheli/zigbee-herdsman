// biome-ignore-all lint/suspicious/noExplicitAny: Tests intentionally pass invalid dynamic values into schema serializers.
...
import {BlzApsOption, BlzEUI64, BlzNodeType, BlzOutgoingMessageType, BlzStatus, BlzValueId} from "../../../../src/adapter/blz/driver/types/named";
...
describe("BLZ Types", () => {
    ...
    describe("LVBytes", () => {
        ...
        it("should serialize Buffer without Buffer.concat", () => {
            expectWithoutBufferConcat(() => LVBytes.serialize(LVBytes, Buffer.from([0xab, 0xcd])), Buffer.from([0x02, 0xab, 0xcd]));
        });
        ...
        it("should deserialize unsigned length-prefixed bytes above 127 bytes", () => {
            ...
            const [value, remaining] = LVBytes.deserialize(LVBytes, Buffer.concat([Buffer.of(0x80), valueBytes, Buffer.of(0xef)]));
    ...
    describe("WordList", () => {
        ...
        it("should serialize without Buffer.concat", () => {
            expectWithoutBufferConcat(() => WordList.serialize(WordList, [0x1234, 0x5678]), Buffer.from([0x34, 0x12, 0x78, 0x56]));
        });

        it("should deserialize list of uint16", () => {
            const [value, remaining] = WordList.deserialize(WordList, Buffer.from([0x34, 0x12, 0x78, 0x56]));
    ...
    describe("Generic lists", () => {
        ...
        it("should serialize without Array.map", () => {
            ...
            expectWithoutArrayMap(() => Uint16List.serialize(Uint16List, [0x1234, 0x5678]), Buffer.from([0x34, 0x12, 0x78, 0x56]));
        });
    ...
    describe("Fixed lists", () => {
        ...
        it("should serialize without Array.map", () => {
            ...
            expectWithoutArrayMap(() => FixedUint16List.serialize(FixedUint16List, [0x1234, 0x5678]), Buffer.from([0x34, 0x12, 0x78, 0x56]));
        });
    });
    ...
    describe("Structured serialization", () => {
        ...
        it("should serialize schemas without Buffer.concat", () => {
            expectWithoutBufferConcat(() => serializeSchema([0x1234, 0x56], [uint16_t, uint8_t]), Buffer.from([0x34, 0x12, 0x56]));
        });

        it("should serialize schemas without Array.map", () => {
            expectWithoutArrayMap(() => serializeSchema([0x1234, 0x56], [uint16_t, uint8_t]), Buffer.from([0x34, 0x12, 0x56]));
        });
    ...
    describe("Named Types", () => {
        describe("BlzEUI64", () => {
            ...
            it("should serialize EUI64 without per-byte integer serialization churn", () => {
                ...
                try {
                    expect(BlzEUI64.serialize(BlzEUI64, eui)).toEqual(Buffer.from([0x08, 0x07, 0x06, 0x05, 0x04, 0x03, 0x02, 0x01]));
