...
describe("BLZFrameData", () => {
    ...
    it("returns undefined when candidate frame parse errors cannot be stringified", () => {
        ...
        try {
            expect(BLZFrameData.createFrame(FRAMES.getValue.ID, false, Buffer.alloc(0))).toBeUndefined();
    ...
    it("serializes command fields without Buffer.concat allocation churn", () => {
        ...
        try {
            ...
            expect(frame.serialize()).toStrictEqual(Buffer.from([BlzValueId.BLZ_VALUE_ID_STACK_VERSION]));
    ...
    it("rejects declared byte fields with truncated payload data", () => {
        expect(() => new BLZFrameData("getValue", false, Buffer.from([0x00, 0x03, 0xaa, 0xbb]))).toThrow(
            "Byte field value expected 3 bytes, received 2",
        );
    });
    ...
    it("rejects trailing bytes after a declared byte field", () => {
        expect(() => new BLZFrameData("getValue", false, Buffer.from([0x00, 0x02, 0xaa, 0xbb, 0xcc]))).toThrow(
            "Unexpected trailing data after getValue frame: 1 bytes",
        );
    });
