...
export function copyBytes(value: ArrayLike<number>, target: Buffer, offset = 0, length = value.length): void {
...
export function fixedBufferFromBytes(value: ArrayLike<number>, length: number, errorMessage: string): Buffer {
...
export function fixedBufferFromHex(value: string, length: number, lengthErrorMessage: string, hexErrorMessage = lengthErrorMessage): Buffer {
    ...
    const result = Buffer.allocUnsafe(length);
    for (let i = 0; i < length; i++) {
        result[i] = (hexNibble(value.charCodeAt(i * 2), hexErrorMessage) << 4) | hexNibble(value.charCodeAt(i * 2 + 1), hexErrorMessage);
    }
...
export function uint64FromLittleEndianBytes(value: ArrayLike<number>): bigint {
    if (value.length < 8) {
        throw new RangeError(`Buffer too small. Expected at least 8 bytes, received ${value.length}`);
    }
