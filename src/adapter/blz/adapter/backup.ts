...
export class BLZAdapterBackup {
    ...
    public async createBackup(assertActive: () => void = () => {}): Promise<Models.Backup> {
        ...
        assertActive();
        const tclKey = fixedBufferFromBytes(linkResult.linkKey, 16, "Trust Center link key must be 16 bytes.");
        const netKey = fixedBufferFromBytes(netResult.nwkKey, 16, "Network key must be 16 bytes.");
        ...
        return {
            ...
            coordinatorIeeeAddress: fixedBufferFromBytes(ieee, 8, "Coordinator IEEE address must be 8 bytes."),
            devices: [],
        };
    }
    ...
    public async getStoredBackup(): Promise<Models.Backup | undefined> {
        ...
        if (backupData.metadata?.format === "zigpy/open-coordinator-backup" && backupData.metadata?.version) {
            if (backupData.metadata?.version !== 1) {
                throw new Error(`Unsupported open coordinator backup version (version=${backupData.metadata?.version})`);
            }
