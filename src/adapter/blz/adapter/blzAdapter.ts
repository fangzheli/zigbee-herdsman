...
import {collectCleanupErrors, runCleanupSteps, throwCollectedErrors} from "../lifecycleCleanup";
import {parseNwkUpdateChannelChange} from "./nwkUpdate";
import {type ZclResponseWaiter, ZclResponseWaiters} from "./zclResponseWaiters";
...
function parseZclHeader(message: Buffer): Zcl.Header | undefined {
    ...
    if (header !== undefined && header.frameControl.frameType !== Zcl.FrameType.GLOBAL && header.frameControl.frameType !== Zcl.FrameType.SPECIFIC) {
        logger.debug(`Ignoring ZCL header with reserved frame type ${header.frameControl.frameType}`, NS);
        return undefined;
    }
...
export class BLZAdapter extends Adapter {
    ...
    private processMessage(frame: BlzIncomingMessage): void {
        ...
        if (frame.apsFrame.profileId === Zdo.ZDO_PROFILE_ID) {
            ...
            const payload: ZclPayload = {
                ...
                wasBroadcast: frame.messageType === BlzOutgoingMessageType.BLZ_MSG_TYPE_BROADCAST,
    ...
    private handleDeviceLeft(nwk: number, ieee: BlzEUI64): void {
        const ieeeAddr = formatIeeeAddress(ieee);
        logger.debug(() => `Device left network request received: ${nwk} ${ieeeAddr}`, NS);
    ...
    private async performStart(): Promise<StartResult> {
        ...
        try {
            this.attachDriverListeners();
            const result = await this.runOperationWhileRunning(() => this.driver.startup(), generation);
            await this.waitWhileRunning(1000, generation);
            return result;
        } catch (error) {
            const startError = errorFromUnknown(error);
            return this.throwAfterFailedStartCleanup(startError);
        }
    }
    ...
    private throwAfterFailedStartCleanup(error: Error): never {
        ...
        if (cleanupErrors.length > 0) {
            throwCollectedErrors([error, ...cleanupErrors], "Failed to start adapter and cleanup driver listeners");
        }
    ...
    private onDriverClose(): void {
        ...
        try {
            ...
            logger.debug(() => `Failed to cleanup after driver close ${formatUnknownError(cleanupError)}`, NS);
        }
    ...
    private async runOperationWhileRunning<T>(operation: () => Promise<T>, generation: number): Promise<T> {
    ...
    private async waitWhileRunning(milliseconds: number, generation: number): Promise<void> {
        this.throwIfStopped(generation);

        const completed = await this.stopDelay.wait(milliseconds, () => this.isRunningGeneration(generation));
    ...
    private async runQueuedWhileRunning<T>(operation: (generation: number) => Promise<T>, key?: string | number): Promise<T> {
    ...
    public getCoordinatorIEEE(): Promise<string> {
        return Promise.resolve(formatIeeeAddress(this.driver.getCoordinatorIeee()));
    }
    ...
    public async permitJoin(seconds: number, networkAddress?: number): Promise<void> {
        ...
        if (networkAddress) {
            ...
            await this.runOperationWhileRunning(() => this.driver.permitJoining(seconds), generation);
    ...
    public getCoordinatorVersion(): Promise<CoordinatorVersion> {
        return Promise.resolve(this.driver.getCoordinatorVersion());
    }

    public addInstallCode(_ieeeAddress: string, _key: Buffer): Promise<void> {
        return Promise.reject(new Error("Not supported"));
    }

    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    public reset(_type: "soft" | "hard"): Promise<void> {
        return Promise.reject(new Error("Not supported"));
    }
    ...
    public async sendZdo<K extends keyof ZdoTypes.RequestToResponseMap>(
        ...
        if (clusterId === Zdo.ClusterId.NWK_UPDATE_REQUEST && ZSpec.Utils.isBroadcastAddress(networkAddress)) {
        ...
        return await this.runQueuedWhileRunning(async (generation) => {
            const response = await this.runOperationWhileRunning(
                () => this.driver.sendZdo(ieeeAddress, networkAddress, clusterId, payload, disableResponse),
                generation,
            );
            this.throwIfStopped(generation);
     ...
     */
    private async handleNwkUpdateRequest(networkAddress: number, clusterId: Zdo.ClusterId, rawPayload: Buffer): Promise<void> {
        logger.debug(() => `[BLZ] NWK_UPDATE_REQUEST raw  len=${rawPayload.length}  ${rawPayload.toString("hex")}`, NS);

        const channelChange = parseNwkUpdateChannelChange(rawPayload, this.hasZdoMessageOverhead);
        ...
        logger.info(`[BLZ] Channel-change request -> channel ${channelChange.channel}, ` + `nwkUpdateId=${channelChange.nwkUpdateId}`, NS);

        await this.runQueuedWhileRunning(async (generation) => {
            await this.runOperationWhileRunning(() => this.driver.sendZdo(ZSpec.BLANK_EUI64, networkAddress, clusterId, payload, true), generation);
            this.throwIfStopped(generation);

            await this.runOperationWhileRunning(() => this.driver.changeChannel(channelChange.channel, channelChange.nwkUpdateId), generation);
            this.throwIfStopped(generation);
        }, networkAddress);
    }
    ...
    public async sendZclFrameToAll(
        ...
        return await this.runQueuedWhileRunning<void>(async (generation) => {
            ...
            const sent = await this.runOperationWhileRunning(
                () =>
                    this.driver.sendZclBroadcast(destination, zclFrame.cluster.ID, resolvedProfileId, sourceEndpoint, endpoint, zclFrame.toBuffer()),
                generation,
            );
            if (!sent) {
    ...
    public getNetworkParameters(): Promise<NetworkParameters> {
        ...
        const extendedPanID = extPanId instanceof Buffer ? `0x${extPanId.toString("hex")}` : "0x0000000000000000";
    ...
    public async backup(): Promise<Models.Backup> {
        ...
        assert(this.driver.isInitialized(), "Cannot make backup when blz is not initialized");
        return await this.runOperationWhileRunning(() => this.driver.createBackup(() => this.throwIfStopped(generation)), generation);
    }
    ...
    public sendZclFrameInterPANToIeeeAddr(_zclFrame: Zcl.Frame, _ieeeAddr: string): Promise<void> {
        return Promise.reject(new Error("Not supported"));
    }
    ...
    public sendZclFrameInterPANBroadcast(_zclFrame: Zcl.Frame, _timeout: number, _disableResponse: boolean): Promise<ZclPayload | undefined> {
        return Promise.reject(new Error("Not supported"));
    }

    public setTransmitPower(_value: number): Promise<void> {
        return Promise.reject(new Error("Not supported"));
    }

    public setChannelInterPAN(_channel: number): Promise<void> {
        return Promise.reject(new Error("Not supported"));
    }
    ...
    private waitForInternal(
        ...
        return this.zclResponseWaiters.waitFor(networkAddress, endpoint, transactionSequenceNumber, clusterID, commandIdentifier, timeout);
    }
