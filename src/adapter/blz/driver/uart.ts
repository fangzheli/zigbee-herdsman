...
import {type UartFrameWaiter, UartFrameWaiters} from "./uartFrameWaiters";
...
export class SerialDriver extends EventEmitter {
    ...
    private async performConnect(options: SerialPortOptions): Promise<void> {
        ...
        if (isTcpPath(path)) {
            ...
            await this.openSerialPort(path, options.baudRate ?? 2000000, options.rtscts ?? false);
        }
    }
        ...
        const openSocket = (): Promise<void> =>
            new Promise<void>((resolve, reject): void => {
                ...
                const detachOpenListeners = (): void => {
                    this.detachSocketOpenListeners(socketPort, onConnect, onReady, openError, openClose);
                };
                ...
                this.attachSocketOpenListeners(socketPort, onConnect, onReady, openError, openClose);
    ...
    private handleError(_frame: Frame): void {
    ...
    private async performClose(): Promise<void> {
        ...
        const connectCleanupError = this.captureConnectOperationCleanup(closeError);
    ...
    private throwAfterCloseCleanup(connectCleanupError: unknown, closeCleanupError: unknown): never {
        if (connectCleanupError !== undefined) {
            throw new AggregateError([connectCleanupError, closeCleanupError], "Failed to cancel connect operations and close UART resources");
        }
    ...
    private async closeActivePort(wasInitialized: boolean, serialPort: SerialPort | undefined): Promise<void> {
        if (serialPort) {
    ...
    private async closeSerialPort(wasInitialized: boolean, serialPort: SerialPort): Promise<void> {
        try {
            await runAsyncCleanupSteps([
                () => {
    ...
    private destroyActivePortAfterCloseFailure(portType: "serial" | "socket", rethrow: boolean): void {
        try {
            this.destroyActivePort();
        } catch (destroyError) {
            logger.debug(() => `Failed to destroy ${portType} port after close failure: ${formatErrorMessage(destroyError)}`, NS);
    ...
    private throwAfterFailedOpenCleanup(port: SerialPort | net.Socket, error: unknown, message: string): never {
        try {
    ...
    private attachSocketOpenListeners(
        ...
        attachListenersOrRollback(port, this.socketOpenListenerRegistrations(onConnect, onReady, onError, onClose));
    }
    ...
    private detachSocketOpenListeners(
        ...
        detachListeners(port, this.socketOpenListenerRegistrations(onConnect, onReady, onError, onClose));
    }
    ...
    private createPortCloseError(err: boolean | Error): Error {
        return err != null && err !== false ? this.createConnectionResetError() : this.createConnectionClosedError();
    }
    ...
    private cleanupPortEvent(error: Error): void {
        try {
            ...
            logger.debug(() => `Failed to cleanup UART port event: ${formatErrorMessage(cleanupError)}`, NS);
        }
    }
    ...
    public async sendDATA(data: Buffer, frameId: number, retries = 2): Promise<void> {
        try {
            return await this.queue.execute(() => this.sendDATAInternal(data, frameId, retries));
        } catch (error) {
            if (error instanceof Error && (error.message === "Connection reset" || error.message === "Connection closed")) {
    ...
    private async sendDATAInternal(data: Buffer, frameId: number, retries: number): Promise<void> {
        ...
        for (let attempt = 0; attempt <= retries; attempt++) {
            ...
            const waiter: UartFrameWaiter | undefined = frameId === FRAMES.reset.ID ? undefined : this.frameWaiters.waitFor(frameId, 1000); // 1 second timeout per attempt

            try {
                this.writer.sendData(data, seq, ackSeq, frameId, isRetransmission, false);
    ...
    private async waitForSendRetry(milliseconds: number, generation: number): Promise<boolean> {
        return await this.sendRetryDelay.wait(milliseconds, () => this.isOperationGenerationActive(generation));
    }
