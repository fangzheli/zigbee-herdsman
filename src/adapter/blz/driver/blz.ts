...
import {type BlzCommandWaiter, BlzCommandWaiters} from "./blzCommandWaiters";
...
import {FRAMES, type ParamsDesc} from "./commands";
import {BLZFrameData} from "./frameData";
import * as t from "./types";
import {type Bytes, uint8_t, uint16_t, type uint32_t, type uint64_t} from "./types";
import {BlzStatus, BlzValueId} from "./types/named";
...
export class Blz extends EventEmitter {
    ...
    private async performConnect(options: SerialPortOptions): Promise<void> {
        ...
        try {
            if (this.serialDriver.isInitialized()) {
                ...
                const runtimeCleanupError = this.captureSerialRuntimeCleanup(reconnectError);
                let closeError: unknown;

                try {
                    await this.runConnectOperation(() => this.serialDriver.close(false), connectGeneration);
                } catch (error) {
                    closeError = error;
                }

                this.throwIfReconnectCleanupFailed(runtimeCleanupError, closeError);
            }

            const resetForReconnect = (): void => {
                this.cancelConnectResetOperations(this.createFailureToConnectError());
            };
            await this.withConnectResetListener(resetForReconnect, () => this.connectWithRetries(options, connectGeneration));
    ...
    private async throwAfterFailedConnectCleanup(error: unknown): Promise<never> {
        try {
            ...
            throw new AggregateError([error, cleanupError], "Failed to connect and cleanup serial driver listeners");
        }
    ...
    private async connectWithRetries(options: SerialPortOptions, connectGeneration: number): Promise<void> {
        ...
        for (let i = 1; i <= MAX_SERIAL_CONNECT_ATTEMPTS; i++) {
            try {
                ...
                if (i < MAX_SERIAL_CONNECT_ATTEMPTS) {
                    ...
                    const continueRetry = await this.waitForConnectRetry(delay, connectGeneration);
    ...
    private async withConnectResetListener(listener: () => void, operation: () => Promise<void>): Promise<void> {
        ...
        if (hasOperationError && hasCleanupError) {
            throw new AggregateError([operationError, cleanupError], "Failed to connect and cleanup connect reset listener");
        }
    ...
    private async runSerialConnectAttempt(options: SerialPortOptions, connectGeneration: number): Promise<void> {
    ...
    private async cleanupFailedConnectAttempt(connectGeneration: number): Promise<void> {
        const connectFailureError = this.createFailureToConnectError();
        const runtimeCleanupError = this.captureSerialRuntimeCleanup(connectFailureError);
        ...
        try {
            ...
            logger.debug(() => `Failed to close serial driver after connect failure: ${formatUnknownError(error)}`, NS);
        }
    ...
    private throwIfReconnectCleanupFailed(runtimeCleanupError: unknown, closeError: unknown): void {
        if (runtimeCleanupError !== undefined && closeError !== undefined) {
            throw new AggregateError([runtimeCleanupError, closeError], "Failed to cleanup serial runtime state and close serial driver");
        }
    ...
    private async runConnectOperation<T>(operation: () => Promise<T>, connectGeneration: number): Promise<T> {
    ...
    private async waitForConnectRetry(milliseconds: number, connectGeneration: number): Promise<boolean> {
        return await this.connectRetryDelay.wait(milliseconds, () => this.isConnectGenerationActive(connectGeneration));
    }
    ...
    private attachSerialDriverEventBridge(): void {
        ...
        attachListenersOrRollback(this.serialDriver, this.serialDriverEventBridgeRegistrations);
    ...
    private detachSerialDriverEventBridge(): void {
        ...
        try {
            detachListeners(this.serialDriver, this.serialDriverEventBridgeRegistrations);
        } finally {
    ...
    private cleanupAfterSerialDriverEvent(connectionError: Error, commandError = connectionError): void {
        try {
            ...
            logger.debug(() => `Failed to cleanup after serial driver event ${formatUnknownError(cleanupError)}`, NS);
        }
    }
     ...
     */
    public async forceReset({holdResetState = false}: ForceResetOptions = {}): Promise<void> {
        ...
        try {
            ...
            this.inResetingProcess = wasResetingProcess;
            logger.error(
                formatLogMessage(() => `Direct UART reset failed: ${error}`),
                NS,
            );
            throw error;
        }
    ...
    private onFrameReceived(data: Buffer): void {
        ...
        if (data.length < 6) {
            logger.error(`Received malformed BLZ frame: expected at least 6 bytes, received ${data.length}`, NS);
            return;
        }
    ...
    public async execCommand(name: string, params?: ParamsDesc): Promise<BLZFrameData> {
        ...
        return await this.queue.execute<BLZFrameData>(async (): Promise<BLZFrameData> => {
            ...
            const waiter: BlzCommandWaiter | undefined = name === "reset" ? undefined : this.commandWaiters.waitFor(name);
            try {
    ...
    async setValue(valueId: t.BlzValueId, value: number | Buffer): Promise<BLZFrameData> {
        ...
        logger.debug(() => `Set ${valueName} = ${this.formatSetValueForLog(value)}`, NS);
    ...
    public async getVersion(): Promise<void> {
        ...
        const verInfo = await this.getValue(BlzValueId.BLZ_VALUE_ID_STACK_VERSION);
        // Update parsing logic if necessary
        const [build, afterBuild] = uint16_t.deserialize(uint16_t, verInfo) as [number, Buffer];
        const [major, afterMajor] = uint8_t.deserialize(uint8_t, afterBuild) as [number, Buffer];
        const [minor, afterMinor] = uint8_t.deserialize(uint8_t, afterMajor) as [number, Buffer];
        const [patch] = uint8_t.deserialize(uint8_t, afterMinor) as [number, Buffer];
