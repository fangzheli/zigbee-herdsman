...
export class Writer extends stream.Readable {
    ...
    public sendData(data: Buffer, seq: number, ackSeq: number, frameId: number, isRetransmission = false, isDebug = false): void {
        const control = this.makeControlByte(isDebug, isRetransmission);
        const frame = this.makeFrame(control, seq, ackSeq, frameId, data);
        this.writeBuffer(frame);
    }

    public sendReset(seq: number, ackSeq: number, isRetransmission = false, isDebug = false): void {
        const control = this.makeControlByte(isDebug, isRetransmission);
    ...
    private makeFrame(control: number, seq: number, ackSeq: number, frameId: number, data?: Buffer): Buffer {
        const sequence = ((ackSeq & 0x07) << 4) | (seq & 0x07);
        return wrapFrameBuffer(buildFrameBuffer(control, sequence, frameId, data));
    }

    private makeControlByte(isDebug: boolean, isRetransmission: boolean): number {
