...
import type {GenericZdoResponse, RequestToResponseMap} from "../../../zspec/zdo/definition/tstypes";
...
import {attachListenersOrRollback, detachListeners, type OwnedEventListener} from "../eventListeners";
import {runAsyncCleanupSteps, runCleanupSteps} from "../lifecycleCleanup";
import {AddressCache, type AddressCacheInput} from "./addressCache";
import {type BLZFrameData, Blz} from "./blz";
...
import {BlzNodeType, BlzStatus, type Bytes, type uint8_t, type uint32_t} from "./types";
...
import {type ZdoResponseWaiter, ZdoResponseWaiters} from "./zdoResponseWaiters";
...
type ApsFrameOverrides = Partial<Pick<BlzApsFrame, "profileId" | "sourceEndpoint" | "destinationEndpoint" | "groupId">>;
...
export class Driver extends EventEmitter {
    ...
    public async createBackup(assertActive: () => void = () => {}): Promise<Models.Backup> {
    ...
    public getNetworkParametersSnapshot(): BlzNetworkParameters {
        ...
        if (Buffer.isBuffer(this.networkParams.extendedPanId)) {
            snapshot.extendedPanId = fixedBufferFromBytes(this.networkParams.extendedPanId, 8, "Network extended PAN ID must be 8 bytes.");
        }
    ...
    private updateNetworkParametersSnapshot(channel: number, nwkUpdateId: number): void {
        if (!this.networkParams) {
            throw new Error("BLZ network parameters are not available");
        }
    ...
    public async changeChannel(newChannel: number, nwkUpdateId: number): Promise<void> {
        ...
        logger.debug(() => `[BLZ]   - ExtendedPanID: 0x${bytesToHex(currentParams.extendedPanId)}`, NS);
        ...
        await this.setNetworkKeyInfo(networkKeyInfo.nwkKey, networkKeyInfo.outgoingFrameCounter, networkKeyInfo.nwkKeySeqNum);
        await this.setGlobalTcLinkKey(tcLinkKeyInfo.linkKey, tcLinkKeyInfo.outgoingFrameCounter);
    ...
    private async throwAfterFailedResetCleanup(error: unknown): Promise<never> {
        try {
            ...
            logger.debug(() => `Failed to stop after failed reset ${cleanupError}`, NS);
            throw new AggregateError([error, cleanupError], "Failed to reset and cleanup BLZ resources");
        }
    ...
    private onBlzReset(): void {
        ...
        void this.reset().catch((error) => {
            logger.error(
                formatLogMessage(() => `BLZ reset recovery failed: ${error}`),
                NS,
            );
        });
    }
    ...
    private cleanupAfterBlzClose(closeError: Error): void {
        try {
            ...
            logger.debug(() => `Failed to cleanup after BLZ close ${formatUnknownError(cleanupError)}`, NS);
        }
    }
    ...
    public async stop(emitClose = true, internalReset = false): Promise<void> {
        if (emitClose) {
    ...
    private async waitForPreparedStop(stopPromise: Promise<void>, prepareError: unknown): Promise<void> {
        try {
            await stopPromise;
        } catch (stopError) {
            if (prepareError !== undefined) {
                throw new AggregateError([prepareError, stopError], "Failed to prepare driver stop and cleanup BLZ resources");
            }
    ...
    private async performStartup(): Promise<TsType.StartResult> {
        ...
        try {
            ...
            try {
                await this.runStartupOperation(() => blz.connect(this.serialOpt), startupStopGeneration);
            } catch (error) {
            ...
            await this.runStartupOperation(() => blz.forceReset(), startupStopGeneration);
            ...
            await this.runStartupOperation(
                () =>
                    this.addEndpoint({
                        ...
                        outputClusters: [0x0000, 0x0003, 0x0004, 0x0005, 0x0006, 0x0008, 0x0020, 0x0300, 0x0400],
                    }),
                startupStopGeneration,
            );

            await this.runStartupOperation(() => blz.getVersion(), startupStopGeneration);
            ...
            if (await this.needsToBeInitialised(this.nwkOpt, startupStopGeneration)) {
                ...
                const restore = await this.runStartupOperation(() => this.needsToBeRestore(this.nwkOpt), startupStopGeneration);

                logger.info("Leaving the current network", NS);

                const st = await this.runStartupOperation(() => blz.leaveNetwork(), startupStopGeneration);
            ...
            logger.debug(() => `Node type: ${netParams.nodeType}, Network parameters: ${networkParams}`, NS);
    ...
    private async throwAfterFailedStartupCleanup(error: unknown): Promise<never> {
        try {
            ...
            throw new AggregateError([error, cleanupError], "Failed to startup and cleanup BLZ resources");
        }
    ...
    private async cleanupFailedStartup(error: unknown): Promise<void> {
        logger.debug(() => `Startup failed, cleaning up BLZ resources: ${formatUnknownError(error)}`, NS);
        const resetCancelledStartup = error === this.startupCancellationError && error instanceof Error && error.message === "Driver reset";
    ...
    private async needsToBeInitialised(options: TsType.NetworkOptions, startupStopGeneration: number): Promise<boolean> {
        const blz = this.getBlz();
        const stackUp = await this.runStartupOperation(() => blz.networkInit(), startupStopGeneration);
        logger.debug(`needToBeInitialized success stack up: ${stackUp}`, NS);
        if (!stackUp) {
            return true;
        }

        const netParams = await this.runStartupOperation(() => blz.execCommand("getNetworkParameters"), startupStopGeneration);
        ...
        logger.debug(`needToBeInitialized success get parameters: ${gotParameters}`, NS);
        if (!gotParameters) {
            return true;
        }
        ...
        const configuredExtendedPanId = options.extendedPanID;
        if (configuredExtendedPanId === undefined) {
            logger.debug("needToBeInitialized missing configured extended PanID", NS);
            return true;
        }

        const extPanIdBytes = uint64ToLittleEndianBuffer(netParams.extPanId);
        const sameExtendedPanId = bytesEqual(configuredExtendedPanId, extPanIdBytes);
        logger.debug(() => `options.extendedPanID: ${bytesToHex(configuredExtendedPanId)}`, NS);
        logger.debug(() => `current extendedPanID: ${bytesToHex(extPanIdBytes)}`, NS);
        logger.debug(`needToBeInitialized same extended PanID: ${sameExtendedPanId}`, NS);
        return !sameExtendedPanId;
    }

    private async formNetwork(restore: boolean, startupStopGeneration?: number): Promise<void> {
        const blz = this.getBlz();
        let backup: Models.Backup | undefined;
        if (restore) {
            backup = await this.runMaybeStartupOperation(() => this.backupMan.getStoredBackup(), startupStopGeneration);
            ...
            await this.runMaybeStartupOperation(() => this.setNetworkKeyInfo(networkKey, frameCounter, sequenceNumber), startupStopGeneration);
            // await this.setGlobalTcLinkKey(backup.blz!.tclk!, backup.blz!.tclkFrameCounter!);
        } else {
            if (this.nwkOpt.networkKey) {
                const networkKey = fixedBufferFromBytes(this.nwkOpt.networkKey, 16, "Network key must be 16 bytes.");
                await this.runMaybeStartupOperation(() => this.setNetworkKeyInfo(networkKey, 0, 0), startupStopGeneration);
            }
        }
        ...
        if (restore) {
            ...
            const backupextendedPanID = uint64FromLittleEndianBytes(backup.networkOptions.extendedPanId);
            formStatus = await this.runMaybeStartupOperation(
                () => blz.formNetwork(backupextendedPanID, backup.networkOptions.panId, backup.logicalChannel),
                startupStopGeneration,
            );
        } else {
            const nwkOptExtendedPanId = this.nwkOpt.extendedPanID;
            if (nwkOptExtendedPanId === undefined) {
                throw new Error("Cannot form network without configured extended PAN ID.");
            }
            const nwkoptextendedPanID = uint64FromLittleEndianBytes(nwkOptExtendedPanId);
            formStatus = await this.runMaybeStartupOperation(
                () => blz.formNetwork(nwkoptextendedPanID, this.nwkOpt.panID, this.nwkOpt.channelList[0]),
                startupStopGeneration,
            );
        }
    ...
    private async runMaybeStartupOperation<T>(operation: () => Promise<T>, startupStopGeneration?: number): Promise<T> {
        ...
        switch (frameName) {
            ...
            case "deviceJoinCallback": {
                ...
                if (frame.status === 0x03) {
                    ...
                    logger.debug(`Device left callback received: nwk=0x${frame.nodeId.toString(16)} ieee=${ieeeAddr}`, NS);
                    this.handleNodeLeft(frame.nodeId, ieeeAddr);
                } else {
                    logger.debug(`Device joined callback received: status=${frame.status}`, NS);
    ...
    private emitIncomingApsMessage(frame: BLZFrameData, apsFrame: BlzApsFrame, options?: {zdoResponse?: GenericZdoResponse}): void {
    ...
    private handleApsDataIndication(frame: BLZFrameData): void {
        ...
        if (frame.profileId === Zdo.ZDO_PROFILE_ID && frame.clusterId >= 0x8000 /* response only */) {
            ...
            try {
                zdoResponse = Zdo.Buffalo.readResponse(true, frame.clusterId, frame.message);
            } catch (error) {
    ...
    private async request(nwk: number | BlzEUI64, apsFrame: BlzApsFrame, data: Buffer): Promise<boolean> {
        ...
        for (let attempt = 0; attempt < REQUEST_ATTEMPT_DELAYS.length; attempt++) {
            ...
            try {
                ...
                const sendResult = await this.runRequestOperation(
                    () => this.sendApsData(BlzOutgoingMessageType.BLZ_MSG_TYPE_UNICAST, resolvedNwk, apsFrame, data),
                    requestGeneration,
                );
                ...
                logger.debug(`Request attempt ${attempt + 1}/${REQUEST_ATTEMPT_DELAYS.length} failed with status=${sendResult}`, NS);
            } catch (e) {
                logger.debug(() => `Request attempt ${attempt + 1}/${REQUEST_ATTEMPT_DELAYS.length} error: ${formatUnknownError(e)}`, NS);
            }
            ...
            if (attempt < REQUEST_ATTEMPT_DELAYS.length - 1) {
                const continueRetry = await this.waitForRequestRetry(REQUEST_ATTEMPT_DELAYS[attempt], requestGeneration);
    ...
    private async runRequestOperation<T>(operation: () => Promise<T>, requestGeneration: number): Promise<T> {
    ...
    private async runBlzCommandOperation<T>(operation: (blz: Blz) => Promise<T>): Promise<T> {
        const blz = this.getBlz();
        const requestGeneration = this.requestGeneration;

        return await this.runRequestOperation(() => operation(blz), requestGeneration);
    }

    private async waitForRequestRetry(milliseconds: number, requestGeneration: number): Promise<boolean> {
        return await this.requestRetryDelay.wait(milliseconds, () => this.isRequestGenerationActive(requestGeneration));
    }

    private async waitForChannelChangeDelay(milliseconds: number): Promise<void> {
        const requestGeneration = this.requestGeneration;
        const completed = await this.channelChangeDelay.wait(milliseconds, () => this.isRequestGenerationActive(requestGeneration));
    ...
    private async waitForResetDelay(milliseconds: number, resetStopGeneration: number): Promise<boolean> {
        if (!this.isResetGenerationActive(resetStopGeneration)) {
            logger.debug("Reset cancelled by stop.", NS);
            return false;
        }

        const stillActive = await this.resetDelay.wait(milliseconds, () => this.isResetGenerationActive(resetStopGeneration));
    ...
    private async waitForStartupDelay(milliseconds: number, startupStopGeneration: number): Promise<void> {
        this.throwIfStartupCancelled(startupStopGeneration);

        const stillActive = await this.startupDelay.wait(milliseconds, () => this.isStartupGenerationActive(startupStopGeneration));
    ...
    private async runStartupOperation<T>(operation: () => Promise<T>, startupStopGeneration: number): Promise<T> {
    ...
    private async runCheckedStartupCommand(
        ...
        const frameResponse = await this.runStartupOperation(
            () => (params === undefined ? this.getBlz().execCommand(command) : this.getBlz().execCommand(command, params)),
            startupStopGeneration,
        );

        logger.info(`Command (${command}) returned: ${frameResponse.status}`, NS);

        this.assertBlzStatus(frameResponse.status, logMessage, errorMessage, " with status=");

        return frameResponse;
    }

    private async mrequest(apsFrame: BlzApsFrame, data: Buffer): Promise<boolean> {
        return await this.sendRoutedApsRequest(BlzOutgoingMessageType.BLZ_MSG_TYPE_MULTICAST, apsFrame.groupId ?? 0, apsFrame, data);
    }

    private async brequest(destination: number, apsFrame: BlzApsFrame, data: Buffer): Promise<boolean> {
        return await this.sendRoutedApsRequest(BlzOutgoingMessageType.BLZ_MSG_TYPE_BROADCAST, destination, apsFrame, data);
    }

    private async sendRoutedApsRequest(messageType: number, destination: number, apsFrame: BlzApsFrame, data: Buffer): Promise<boolean> {
        this.getBlz();
        const requestGeneration = this.requestGeneration;

        try {
            return await this.runRequestOperation(() => this.sendApsDataStatus(messageType, destination, apsFrame, data), requestGeneration);
        } catch (error) {
            if (this.isRequestCancelled(requestGeneration)) {
                return false;
            }
    ...
    public async sendZclMulticast(groupID: number, clusterId: number, profileId: number, sourceEndpoint: number, data: Buffer): Promise<boolean> {
    ...
    public async sendZdo<K extends keyof RequestToResponseMap>(
        ...
        await this.sendZdoFrame(ieeeAddress, networkAddress, clusterName, frame, requestPayload, waiter, requestGeneration);

        try {
            if (clusterId === Zdo.ClusterId.LEAVE_REQUEST) {
                logger.info(`[BLZ] LEAVE_REQUEST sent to ${ieeeAddress}:${networkAddress}, emitting deviceLeave`, NS);
            ...
            if (waiter && responseClusterId !== undefined) {
                ...
                const zdoResponse = response.zdoResponse;
                if (zdoResponse === undefined) {
                    throw new Error(`Missing ZDO response for cluster ${Zdo.ClusterId[responseClusterId]}`);
                }

                logger.debug(() => `<~~ [ZDO ${Zdo.ClusterId[responseClusterId]} ${JSON.stringify(zdoResponse)}]`, NS);

                return zdoResponse as RequestToResponseMap[K];
            }
        } catch (error) {
    ...
    private async sendZdoFrame(
        ...
        const route = isBroadcast ? `BROADCAST to=${networkAddress}` : `UNICAST to=${ieeeAddress}:${networkAddress}`;

        logger.debug(() => `~~~> [ZDO ${clusterName} ${route} payload=${payload.toString("hex")}]`, NS);

        try {
            const req = await (isBroadcast ? this.brequest(networkAddress, frame, payload) : this.request(networkAddress, frame, payload));
    ...
    private async sendApsDataStatus(messageType: number, destination: number, apsFrame: BlzApsFrame, data: Buffer): Promise<boolean> {
        try {
            return (await this.sendApsData(messageType, destination, apsFrame, data)) === BlzStatus.SUCCESS;
        } catch {
            return false;
        }
    }

    private async sendApsData(messageType: number, destination: number, apsFrame: BlzApsFrame, data: Buffer): Promise<BlzStatus> {
    ...
    private makeApsFrame(clusterId: number, overrides: ApsFrameOverrides = {}): BlzApsFrame {
    ...
    // biome-ignore lint/correctness/noUnusedPrivateClassMembers: Tests exercise this private BLZ lookup path directly.
    ...
    private async formNetworkWithParameters(extendedPanId: bigint, panId: number, channel: number): Promise<BlzStatus> {
        return await this.runBlzCommandOperation((blz) => blz.formNetwork(extendedPanId, panId, channel));
    }
    ...
    private assertBlzStatus(status: BlzStatus, logMessage: string, errorMessage: string, statusText = ": status "): void {
    ...
    private async runCheckedBlzCommand(
        ...
        const frameResponse = await this.runBlzCommandOperation((blz) =>
            params === undefined ? blz.execCommand(command) : blz.execCommand(command, params),
        );

        this.assertBlzStatus(frameResponse.status, logMessage, errorMessage);
    ...
    private async setGlobalTcLinkKey(linkKey: Bytes, outgoingFrameCounter: uint32_t): Promise<BlzStatus> {
        const frameRequest = {
            linkKey,
        ...
        logger.debug(() => `Global TC Key set successfully: Key=${linkKey}, FrameCounter=${outgoingFrameCounter}`, NS);
    ...
    private async setNetworkKeyInfo(nwkKey: Bytes, outgoingFrameCounter: uint32_t, nwkKeySeqNum: uint8_t): Promise<BlzStatus> {
    ...
    private async needsToBeRestore(options: TsType.NetworkOptions): Promise<boolean> {
        ...
        if (currentExtendedPanID === undefined) {
            logger.debug("needsToBeRestore missing configured extendedPanID", NS);
            return false;
        }
        const backupExtendedPanID = backup.networkOptions.extendedPanId;
        logger.debug(() => `Configured extendedPanID (raw): ${bytesToHex(currentExtendedPanID)}`, NS);
        logger.debug(() => `Backup extendedPanID (raw): ${bytesToHex(backupExtendedPanID)}`, NS);
        ...
        if (currentNetworkKey === undefined) {
            logger.debug("needsToBeRestore missing configured network key", NS);
            return false;
        }
        const backupNetworkKey = backup.networkOptions.networkKey;
        logger.debug(() => `Configured networkKey (raw): ${bytesToHex(currentNetworkKey)}`, NS);
        logger.debug(() => `Backup networkKey (raw): ${bytesToHex(backupNetworkKey)}`, NS);
